package edu.romoshi.handler.jdbc.accounts;
...
public class AccountsQuery {
    ...
    public static final String READ = "SELECT name_service, login, password FROM accounts WHERE accounts.user_id = (SELECT users.id FROM users WHERE users.user_info = ?);";
